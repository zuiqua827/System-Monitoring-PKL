                ...
                <div class="mt-2.5 space-y-1">
                    @foreach ($section['items'] as $item)
                        ...
                        <a
                            ...
                            class="group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] font-medium transition-all duration-150 {{ $isActive ? 'sidebar-nav-item-active bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-900/30' : 'sidebar-nav-item-inactive text-slate-400 hover:text-white' }}"
                        >
                            @if($isActive)
                                <span class="absolute left-0 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-white"></span>
                            @endif
                            <svg class="h-5 w-5 shrink-0 transition-colors {{ $isActive ? 'text-white' : 'text-slate-500 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.7">
                                @switch($item['icon'])
                                    @case('dashboard')
