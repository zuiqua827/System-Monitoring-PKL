    ...
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Data Kelas</h1>
                <p class="mt-2 text-sm text-slate-500">Kelola data kelas</p>
            </div>
            @can('create', App\Models\Kelas::class)
                <a href="{{ route('admin.kelas.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card-sm">
                    ...
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-slate-500">
                                <a href="{{ route('admin.kelas.index', array_merge(request()->query(), ['sort' => 'nama', 'direction' => request('sort') === 'nama' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="inline-flex items-center gap-1 hover:text-slate-700">
                                    Nama Kelas
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Jurusan</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($kelass as $index => $kelas)
                            <tr class="transition hover:bg-slate-50">
                                <td class="px-4 py-3.5 text-sm text-slate-500">{{ $kelass->firstItem() + $index }}</td>
                                <td class="px-4 py-3.5 text-sm font-medium text-slate-900">{{ $kelas->nama }}</td>
                                <td class="px-4 py-3.5 text-sm text-slate-600">{{ $kelas->tingkat }}</td>
                                <td class="px-4 py-3.5 text-sm font-medium text-slate-800">{{ $kelas->tahun_ajaran }}</td>
                                <td class="px-4 py-3.5 text-sm text-slate-600">{{ $kelas->jurusan?->nama ?? '-' }}</td>
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1">
                                        @can('view', $kelas)
                                            <a href="{{ route('admin.kelas.show', $kelas->id) }}" class="inline-flex items-center rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">Detail</a>
                                        @endcan

                                        @unless($kelas->trashed())
                                            @can('update', $kelas)
                                                <a href="{{ route('admin.kelas.edit', $kelas->id) }}" class="inline-flex items-center rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700 transition hover:bg-amber-100">Edit</a>
                                            @endcan

                                            @can('delete', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.destroy', $kelas->id) }}" class="inline"
                                        ...
                                        @else
                                            @can('restore', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.restore', $kelas->id) }}" class="inline"
                                            ...
                                            @can('forceDelete', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.force-delete', $kelas->id) }}" class="inline"
            ...
            @if($kelass->hasPages())
                <div class="border-t border-slate-200 px-5 py-4">
                    {{ $kelass->appends(request()->query())->links() }}
                </div>
            @endif
        </div>
    </div>
