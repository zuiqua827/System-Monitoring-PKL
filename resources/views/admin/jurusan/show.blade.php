    ...
    <div class="mx-auto max-w-5xl">
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{{ $jurusan->nama }}</h1>
                <p class="mt-2 text-sm text-slate-500">Detail lengkap data jurusan</p>
            </div>
            <a href="{{ route('admin.jurusan.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-card-sm transition hover:bg-slate-50">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-card-sm">
                <div class="border-b border-slate-100 px-6 py-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Data lengkap jurusan</p>
                </div>
                <div class="grid gap-px overflow-hidden rounded-b-2xl bg-slate-100 sm:grid-cols-1 sm:grid-cols-2">
                    <div class="bg-white p-5 sm:col-span-2">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Nama Jurusan</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $jurusan->nama }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Kode Jurusan</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $jurusan->kode ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                    ...
                    <div class="bg-white p-5 sm:col-span-2">
                        ...
                        <p class="mt-1 text-sm text-slate-700">{{ $jurusan->deskripsi ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tanggal Dibuat</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $jurusan->created_at ? $jurusan->created_at->format('d/m/Y H:i') : '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Terakhir Diupdate</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $jurusan->updated_at ? $jurusan->updated_at->format('d/m/Y H:i') : '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                ...
                @unless($jurusan->trashed())
                    @can('update', $jurusan)
                        <a href="{{ route('admin.jurusan.edit', $jurusan->id) }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-card-sm transition hover:bg-amber-600">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @can('delete', $jurusan)
                        <form method="POST" action="{{ route('admin.jurusan.destroy', $jurusan->id) }}"
                              data-confirm="Hapus jurusan {{ $jurusan->nama }}? Data akan dipindahkan ke tempat sampah."
                ...
                @else
                    @can('restore', $jurusan)
                        <form method="POST" action="{{ route('admin.jurusan.restore', $jurusan->id) }}"
                              data-confirm="Pulihkan jurusan {{ $jurusan->nama }}?"
                              data-confirm-title="Pulihkan Jurusan"
                    ...
                    @can('forceDelete', $jurusan)
                        <form method="POST" action="{{ route('admin.jurusan.force-delete', $jurusan->id) }}"
                              data-confirm="Hapus permanen {{ $jurusan->nama }}? Tindakan ini tidak dapat dibatalkan!"
                              data-confirm-title="Hapus Permanen Jurusan"
                              data-confirm-type="danger"
