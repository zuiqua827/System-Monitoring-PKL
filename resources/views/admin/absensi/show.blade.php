...
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.absensi.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 transition">
                Kembali
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-800">{{ $absensi->penempatanPKL?->periodePKL?->nama ?? '-' }}</p>
                    </div>
                    <div>
            ...
            <div class="flex items-center justify-end gap-3">
                @unless($absensi->trashed())
                    @can('update', $absensi)
                        <a href="{{ route('admin.absensi.edit', $absensi->id) }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 transition">
                            Edit Absensi
                        </a>
                    @endcan
                    @can('delete', $absensi)
                ...
                @else
                    ...
                    @can('forceDelete', $absensi)
                        <form method="POST" action="{{ route('admin.absensi.force-delete', $absensi->id) }}" class="inline" onsubmit="return confirm('Hapus permanen absensi ini? Tindakan ini tidak dapat dibatalkan!')">
                            @csrf
                            @method('DELETE')
