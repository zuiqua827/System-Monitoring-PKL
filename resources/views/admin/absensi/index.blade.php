    ...
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Absensi PKL</h1>
                <p class="mt-2 text-sm text-slate-500">Kelola absensi harian siswa PKL</p>
            </div>
            @can('create', App\Models\Absensi::class)
                <a href="{{ route('admin.absensi.create') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card-sm">
            <form method="GET" action="{{ route('admin.absensi.index') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <div class="relative">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request('search') || request('tanggal') || request('status') || request('periode_id'))
                        <a href="{{ route('admin.absensi.index') }}" class="inline-flex items-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Reset</a>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-slate-500">
                                <a href="{{ route('admin.absensi.index', array_merge(request()->query(), ['sort' => 'status', 'direction' => request('sort') === 'status' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="inline-flex items-center gap-1 hover:text-slate-700">
                                    Status
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($absensis as $index => $absensi)
                            <tr class="transition hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3.5 text-sm text-slate-600">{{ $absensi->penempatanPKL?->guru?->nama ?? '-' }}</td>
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1">
                                        @can('view', $absensi)
                                            <a href="{{ route('admin.absensi.show', $absensi->id) }}" class="inline-flex items-center rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">Detail</a>
                                        @endcan

                                        @unless($absensi->trashed())
                                            @can('update', $absensi)
                                                <a href="{{ route('admin.absensi.edit', $absensi->id) }}" class="inline-flex items-center rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700 transition hover:bg-amber-100">Edit</a>
                                            @endcan

                                            @can('delete', $absensi)
                                                <form method="POST" action="{{ route('admin.absensi.destroy', $absensi->id) }}" class="inline"
                                        ...
                                        @else
                                            @can('restore', $absensi)
                                                <form method="POST" action="{{ route('admin.absensi.restore', $absensi->id) }}" class="inline"
                                            ...
                                            @can('forceDelete', $absensi)
                                                <form method="POST" action="{{ route('admin.absensi.force-delete', $absensi->id) }}" class="inline"
