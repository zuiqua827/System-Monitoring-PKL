    ...
    <div class="mx-auto max-w-5xl">
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{{ $kelas->nama }}</h1>
                <p class="mt-2 text-sm text-slate-500">Detail lengkap data kelas</p>
            </div>
            <a href="{{ route('admin.kelas.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-card-sm transition hover:bg-slate-50">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-card-sm">
                <div class="border-b border-slate-100 px-6 py-5">
                    <h3 class="text-base font-bold text-slate-900">Informasi Kelas</h3>
                    <p class="mt-1 text-sm text-slate-500">Data lengkap kelas</p>
                </div>
                <div class="grid gap-px overflow-hidden rounded-b-2xl bg-slate-100 sm:grid-cols-1 sm:grid-cols-2">
                    <div class="bg-white p-5 sm:col-span-2">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Nama Kelas</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $kelas->nama }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tingkat</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $kelas->tingkat ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Jurusan</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $kelas->jurusan?->nama ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tahun Ajaran</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $kelas->tahun_ajaran ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                    ...
                    <div class="bg-white p-5">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $kelas->created_at ? $kelas->created_at->format('d/m/Y H:i') : '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                <a href="{{ route('admin.kelas.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-card-sm transition hover:bg-slate-50">
                    Kembali
                </a>
                @unless($kelas->trashed())
                    @can('update', $kelas)
                        <a href="{{ route('admin.kelas.edit', $kelas->id) }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-card-sm transition hover:bg-amber-600">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @can('delete', $kelas)
                        <form method="POST" action="{{ route('admin.kelas.destroy', $kelas->id) }}"
                              data-confirm="Hapus kelas {{ $kelas->nama }}? Data akan dipindahkan ke tempat sampah."
                ...
                @else
                    @can('restore', $kelas)
                        <form method="POST" action="{{ route('admin.kelas.restore', $kelas->id) }}"
                              data-confirm="Pulihkan kelas {{ $kelas->nama }}?"
                              data-confirm-title="Pulihkan Kelas"
                    ...
                    @endcan
                    @can('forceDelete', $kelas)
                        <form method="POST" action="{{ route('admin.kelas.force-delete', $kelas->id) }}"
                              data-confirm="Hapus permanen {{ $kelas->nama }}? Tindakan ini tidak dapat dibatalkan!"
                              data-confirm-title="Hapus Permanen Kelas"
                              data-confirm-type="danger"
