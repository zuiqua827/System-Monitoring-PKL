...
<div class="px-4 py-4 sm:px-6 sm:py-8 lg:px-8">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card-sm">
            <form method="GET" action="{{ route('admin.guru.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center flex-wrap">
                <div class="flex-1">
                    <div class="relative">
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        @unless($guru->trashed())
                                            ...
                                            @can('delete', $guru)
                                                <form method="POST" action="{{ route('admin.guru.destroy', $guru->id) }}" class="inline"
                                                      data-confirm="Hapus guru {{ $guru->nama }}? Data akan dipindahkan ke tempat sampah."
                                        ...
                                        @else
                                            @can('restore', $guru)
                                                <form method="POST" action="{{ route('admin.guru.restore', $guru->id) }}" class="inline"
                                                      data-confirm="Pulihkan guru {{ $guru->nama }}?"
                                                      data-confirm-title="Pulihkan Guru"
                                            ...
                                            @can('forceDelete', $guru)
                                                <form method="POST" action="{{ route('admin.guru.force-delete', $guru->id) }}" class="inline"
                                                      data-confirm="Hapus permanen {{ $guru->nama }}? Tindakan ini tidak dapat dibatalkan!"
                                                      data-confirm-title="Hapus Permanen Guru"
                                                      data-confirm-type="danger"
