    ...
    <div class="mx-auto max-w-5xl">
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{{ $siswa->nama }}</h1>
                <p class="mt-2 text-sm text-slate-500">Detail lengkap data siswa peserta PKL</p>
            </div>
            <a href="{{ route('admin.siswa.index') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-card-sm transition hover:bg-slate-50">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-card-sm sm:p-8">
                <div class="flex flex-col gap-6 sm:flex-row sm:items-center">
                    <div class="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 text-2xl font-extrabold text-white shadow-lg shadow-blue-500/20">
                        {{ strtoupper(substr($siswa->nama, 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-3">
                            <h2 class="text-xl font-bold text-slate-900">{{ $siswa->nama }}</h2>
                            <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold {{ $siswa->trashed() ? 'bg-red-100 text-red-800' : 'bg-emerald-100 text-emerald-800' }}">
                                {{ $siswa->trashed() ? 'Dihapus' : ($siswa->user?->email_verified_at ? 'Terverifikasi' : 'Belum Verifikasi') }}
                            </span>
                        </div>
                        <div class="mt-2 grid gap-x-8 gap-y-1 text-sm text-slate-500 sm:grid-cols-2">
                            <p><span class="font-semibold text-slate-700">NIS:</span> {{ $siswa->nis }}</p>
                            <p><span class="font-semibold text-slate-700">NISN:</span> {{ $siswa->nisn ?? '-' }}</p>
                            <p><span class="font-semibold text-slate-700">Kelas:</span> {{ $siswa->kelas?->nama ?? '-' }}</p>
                            <p><span class="font-semibold text-slate-700">Email:</span> {{ $siswa->user?->email ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-card-sm">
                ...
                <div class="grid gap-px overflow-hidden rounded-b-2xl bg-slate-100 sm:grid-cols-2">
                    <div class="bg-white p-5">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->user?->email ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                    ...
                    <div class="bg-white p-5">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->user?->last_login_at ? $siswa->user->last_login_at->format('d/m/Y H:i') : 'Belum pernah' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tanggal Dibuat</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->created_at ? $siswa->created_at->format('d/m/Y H:i') : '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-card-sm">
                <div class="border-b border-slate-100 px-6 py-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Informasi lengkap data siswa</p>
                </div>
                <div class="grid gap-px overflow-hidden rounded-b-2xl bg-slate-100 sm:grid-cols-2">
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">NIS</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->nis }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">NISN</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->nisn ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5 sm:col-span-2">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Nama Lengkap</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->nama }}</p>
                    </div>
                    <div class="bg-white p-5">
                    ...
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Kelas</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->kelas?->nama ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tanggal Lahir</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->tanggal_lahir ? $siswa->tanggal_lahir->format('d/m/Y') : '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">No. Telepon</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->no_telepon ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5 sm:col-span-2">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Alamat</p>
                        <p class="mt-1 text-sm text-slate-700">{{ $siswa->alamat ?? '-' }}</p>
                    </div>
                    <div class="bg-white p-5">
                    ...
                    <div class="bg-white p-5">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">{{ $siswa->penempatan_count ?? $siswa->penempatan()->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                ...
                @unless($siswa->trashed())
                    @can('update', $siswa)
                        <a href="{{ route('admin.siswa.edit', $siswa->id) }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-card-sm transition hover:bg-amber-600">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
