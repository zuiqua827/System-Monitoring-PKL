...
<div class="mx-auto max-w-3xl">
    <form method="POST" action="{{ $route }}" class="space-y-6">
                ...
                @else
                    <div class="bg-white p-5 sm:col-span-2">
                        ...
                        <select id="penempatan_pkl_id" name="penempatan_pkl_id" required
                            ...
                            @foreach(\App\Models\PenempatanPKL::with(['siswa', 'guru', 'dudi', 'periodePKL'])->whereDoesntHave('penilaian')->where('guru_id', auth()->user()->guru?->id ?? 0)->get() as $p)
        ...
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
            <a href="{{ route('guru.penilaian.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-card-sm transition hover:bg-slate-50">
                Batal
            </a>
