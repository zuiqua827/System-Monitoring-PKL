    ...
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Penilaian PKL</h1>
                <p class="mt-2 text-sm text-slate-500">Kelola penilaian PKL siswa</p>
            </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card-sm">
            <form method="GET" action="{{ route('admin.penilaian.index') }}" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <div class="relative">
                    ...
                    <input type="text" name="search" placeholder="Cari siswa, guru, perusahaan..." value="{{ request('search') }}"
                ...
                <div>
                    <select name="guru_id" class="block w-full rounded-xl border border-slate-200 bg-white py-2.5 px-4 text-sm text-slate-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request()->anyFilled(['search','status','guru_id','periode_id']))
                        <a href="{{ route('admin.penilaian.index') }}" class="inline-flex items-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">Reset</a>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3.5 text-left text-xs font-bold uppercase tracking-wide text-slate-500">
                                <a href="{{ route('admin.penilaian.index', array_merge(request()->query(), ['sort' => 'status', 'direction' => request('sort') === 'status' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="inline-flex items-center gap-1 hover:text-slate-700">
                                    Status
                            ...
                            <tr class="transition hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1">
                                        @can('view', $penilaian)
                                            <a href="{{ route('admin.penilaian.show', $penilaian->id) }}" class="inline-flex items-center rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs font-semibold text-blue-700 transition hover:bg-blue-100">Detail</a>
                                        @endcan

                                        @unless($penilaian->trashed())
                                            @can('update', $penilaian)
                                                <a href="{{ route('admin.penilaian.edit', $penilaian->id) }}" class="inline-flex items-center rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-semibold text-amber-700 transition hover:bg-amber-100">Edit</a>
                                            @endcan

                                            @can('delete', $penilaian)
                                                <form method="POST" action="{{ route('admin.penilaian.destroy', $penilaian->id) }}" class="inline"
                                                      data-confirm="Hapus penilaian PKL ini? Data akan dipindahkan ke tempat sampah."
                                        ...
                                        @else
                                            @can('restore', $penilaian)
                                                <form method="POST" action="{{ route('admin.penilaian.restore', $penilaian->id) }}" class="inline"
                                                      data-confirm="Pulihkan penilaian PKL ini?"
                                                      data-confirm-title="Pulihkan Penilaian"
                                            ...
                                            @can('forceDelete', $penilaian)
                                                <form method="POST" action="{{ route('admin.penilaian.force-delete', $penilaian->id) }}" class="inline"
                                                      data-confirm="Hapus permanen penilaian PKL ini? Tindakan ini tidak dapat dibatalkan!"
                                                      data-confirm-title="Hapus Permanen Penilaian"
                                                      data-confirm-type="danger"
