...
<div class="px-4 py-4 sm:px-6 sm:py-8 lg:px-8">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card-sm">
            <form method="GET" action="{{ route('admin.penempatan-pkl.index') }}" class="space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:grid-cols-5">
                    <div>
                    ...
                    <div>
                        <label for="kelas_id" class="mb-1 block text-xs font-semibold text-slate-600">Kelas</label>
                    ...
                    <div>
                        <label for="dudi_id" class="mb-1 block text-xs font-semibold text-slate-600">DUDI</label>
                        <select name="dudi_id" id="dudi_id"
                                class="block w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200">
                            <option value="">Semua DUDI</option>
                            @foreach($dudis as $dudi)
                                <option value="{{ $dudi->id }}" {{ request('dudi_id') == $dudi->id ? 'selected' : '' }}>{{ $dudi->nama_perusahaan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="guru_id" class="mb-1 block text-xs font-semibold text-slate-600">Guru Pembimbing</label>
...
<span>{{ $penempatanPkl->siswa?->nama ?? '-' }}</span>
                                ...
                                <td class="px-4 py-3.5 text-sm text-slate-600">{{ $penempatanPkl->siswa?->kelas?->nama ?? '-' }}</td>
                                    ...
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        @unless($penempatanPkl->trashed())
                                            ...
                                            @can('delete', $penempatanPkl)
                                                <form method="POST" action="{{ route('admin.penempatan-pkl.destroy', $penempatanPkl->id) }}" class="inline"
                                        ...
                                        @else
                                            @can('restore', $penempatanPkl)
                                                <form method="POST" action="{{ route('admin.penempatan-pkl.restore', $penempatanPkl->id) }}" class="inline"
                                            ...
                                            @can('forceDelete', $penempatanPkl)
                                                <form method="POST" action="{{ route('admin.penempatan-pkl.force-delete', $penempatanPkl->id) }}" class="inline"
