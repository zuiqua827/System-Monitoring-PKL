...
<div class="px-4 py-8 sm:px-6 lg:px-8">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-card-sm">
            <form method="GET" action="{{ route('admin.kelas.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div class="flex-1">
                    <div class="relative">
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        @unless($kelas->trashed())
                                            ...
                                            @can('delete', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.destroy', $kelas->id) }}" class="inline" onsubmit="return confirm('Hapus kelas {{ $kelas->nama }}?')">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        @else
                                            @can('restore', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.restore', $kelas->id) }}" class="inline" onsubmit="return confirm('Pulihkan kelas {{ $kelas->nama }}?')">
                                                    @csrf
                                            ...
                                            @can('forceDelete', $kelas)
                                                <form method="POST" action="{{ route('admin.kelas.force-delete', $kelas->id) }}" class="inline" onsubmit="return confirm('Hapus permanen {{ $kelas->nama }}?')">
                                                    @csrf
                                                    @method('DELETE')
